    ...
    <div className="grid grid-cols-3 gap-4 max-[900px]:grid-cols-1">
      {QTY_TIERS.map((tier) => (
        <button
          ...
          className={cn(
            'rounded-2xl border border-[#1D2530] bg-[#0B1016] p-[22px] text-center transition-all duration-200 hover:scale-[1.02]',
        ...
        >
          ...
          <div className={cn('mt-1 text-[13px]', value === tier ? 'text-primary' : 'text-[#9AA1AC]')}>제작</div>
        </button>
      ))}
    </div>
  );
