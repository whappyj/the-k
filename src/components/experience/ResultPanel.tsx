    ...
    <div className="sticky top-6">
      <Card className="p-8">
        <div className="mb-6 flex items-center gap-2.5">
          ...
          <span className="text-[16px] font-bold text-white">결과</span>
        </div>
        ...
        <div className="mb-6 rounded-2xl border border-primary/25 bg-gradient-to-br from-primary-dim to-transparent p-8 text-center">
          <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-primary/80">시간당 경험치</div>
          <div className="font-display text-[68px] font-bold leading-none text-primary">
            {stats ? formatPercent(stats.expPerHour) : '-'}
            <span className="text-[24px] text-primary/70">/h</span>
          </div>
        </div>
        ...
        <div className="mb-6 grid grid-cols-3 gap-4">
          ...
          <Stat icon={TrendingUp} tone="blue" label="분당 경험치" value={stats ? formatPercent(stats.expPerMinute) : '-'} />
        </div>

        <div className="mb-1.5 text-[12px] font-bold text-text-sub">시간별 환산</div>
        <div className="mb-6 grid grid-cols-4 gap-2 max-[480px]:grid-cols-3">
          {PROJECTION_MINUTES.map((m) => (
            <div key={m} className={`rounded-xl border p-2.5 text-center ${m === 1440 ? 'border-primary/40 bg-primary-dim' : 'border-[#1D2530] bg-white/[0.02]'}`}>
              <div className="mb-1 text-[10px] text-text-faint">{PROJECTION_LABEL[m]}</div>
              <div className={`font-display text-[11.5px] font-bold ${m === 1440 ? 'text-primary' : 'text-white'}`}>{formatPercent((perHour * m) / 60)}</div>
            </div>
          ))}
        </div>
