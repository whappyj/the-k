...
export function ExpVelocityDashboard({ records }: { records: ExperienceRecord[] }) {
  ...
  if (!records.length) {
    return (
      <Card className="py-14 text-center text-[13px] text-text-faint">
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-4 max-[900px]:grid-cols-1">
        {windows.map((w) => {
          ...
          return (
            <Card key={w.key} className={cn(!w.hasData && 'opacity-60')}>
              ...
              <div className="mt-3 border-t border-border/[0.08] pt-3">
