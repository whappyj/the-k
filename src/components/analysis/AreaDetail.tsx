...
export function AreaDetail({ area, records }: { area: string; records: ExperienceRecord[] }) {
  ...
  return (
    <div className="mt-4 rounded-2xl border border-[#1D2530] bg-white/[0.03] p-4">
      <div className="mb-2 text-[13px] font-bold">"{area}" 최근 기록</div>
      {list.map((r) => (
        <div key={r.id} className="flex items-center justify-between border-b border-[#1D2530] py-2.5 text-[13px] last:border-none">
          <div>
